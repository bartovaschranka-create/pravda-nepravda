...
const files = {
  ...
  app: "app.js",
  api: "api/research.js",
  personCandidatesApi: "api/person-candidates.js",
  server: "server.mjs",
  styles: "styles.css",
  readme: "README.md"
};
...
function parseCheck(file, source) {
  ...
  if (file === files.api || file === files.personCandidatesApi) {
...
assert(html.includes("person-resolution"), "HTML nemá panel pro výběr konkrétní osoby.");
...
assert(app.includes("BLOCKED_SOURCE_DOMAINS"), "Frontend neumí blokovat problematické přímé odkazy.");
assert(app.includes("isBlockedSourceUrl"), "Frontend neumí převést problematický přímý odkaz na záložní hledání.");
...
assert((await text(files.api)).includes("dateFromText"), "Backend neumí vytáhnout datum z českého textu nebo snippetu.");
assert((await text(files.api)).includes("CZECH_MONTHS"), "Backend neumí české názvy měsíců při hledání data.");
assert((await text(files.api)).includes("dc.date.issued"), "Backend nekontroluje běžná metadata s datem publikace.");
assert((await text(files.api)).includes("SOURCE_PRIORITY_DOMAINS"), "Backend nezvýhodňuje seriózní redakční zdroje.");
assert((await text(files.api)).includes("UNSAFE_SOURCE_DOMAINS"), "Backend neumí vyřadit zdroje s problematickým certifikátem.");
assert((await text(files.api)).includes("isBlockedSource"), "Backend nemá kontrolu problematických přímých odkazů.");
...
assert(app.includes("shouldGatePersonSelection"), "Frontend neumí zastavit pomalé hledání před výběrem osoby.");
assert(app.includes("renderPersonSelectionGate"), "Frontend neumí zobrazit preflight výběr osoby.");
assert(app.includes("PERSON_CANDIDATES_ENDPOINT"), "Frontend neumí načíst kandidáty osoby z backendu.");
assert(app.includes("fetchPersonCandidates"), "Frontend nemá krok dohledání možných osob před hlavní rešerší.");
assert(app.includes("showPersonResolution"), "Frontend neumí zobrazit výběrové okno pro konkrétní osobu.");
assert(app.includes("confirmedPersonSignature"), "Frontend neumí po výběru osoby pustit hlavní hledání bez opakované brány.");
assert(app.includes("forcePersonCandidates"), "Profil neumí vynutit výběr i při jedné možné shodě.");
...
assert((await text(files.personCandidatesApi)).includes("api.search.brave.com"), "Backend pro kandidáty osob nepoužívá Brave Search.");
assert((await text(files.personCandidatesApi)).includes("candidateRole"), "Backend pro kandidáty osob nevrací stručný popis shody.");
assert((await text(files.server)).includes("/api/person-candidates"), "Server nemá endpoint pro dohledání kandidátů osoby.");
...
assert(styles.includes(".person-resolution"), "Chybí styl pro panel výběru konkrétní osoby.");
assert(styles.includes(".resolution-card"), "Chybí styl pro karty nalezených osob.");
...
parseCheck(files.personCandidatesApi, await text(files.personCandidatesApi));
